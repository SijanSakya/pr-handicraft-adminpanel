...
interface TextareaProps {
  ...
  defaultValue?: string; // Current defaultValue
...
const TextArea: React.FC<TextareaProps> = ({
  ...
  if (disabled) {
    textareaClasses += ` bg-gray-100 opacity-50 text-gray-500 border-gray-300 cursor-not-allowed dark:bg-gray-800 dark:text-gray-800 dark:border-gray-700`;
  } else if (error) {
  ...
  return (
    <div className="relative">
      ...
      {errorMessage && <p className={`absolute -bottom-3 mt-1.5 text-xs text-error-500`}>{errorMessage}</p>}
    </div>
  );
};
