...
const createProductCategoriesSchema = z.object({
  category_title: z.string().min(3, { message: 'Product Category required' }),
  is_main_collection: z.boolean().optional(),
  image: z.any(),
});

const CreateProductCategories = ({ isEditing, editingDetails, refetch, closeModal }: createCateProps) => {
  type Inputs = z.infer<typeof createProductCategoriesSchema>;
  const {
    register,
    handleSubmit,
    watch,
